...
class Indexlist {
 ...
 public:
  ...
  const std::vector<int>& getNumberArray() const { return numberArray; }

  /** Determines the index within the index list at with a given number is
   stored. *  \return >= 0: Index of given number. \n -1: Number not found. */
  int getIndex(int givennumber /**< Number whose index shall be determined. */
               ) const;

  /** Determines the physical index within the index list at with a given number
   is stored. *  \return >= 0: Index of given number. \n -1: Number not found.
 */
  int getPhysicalIndex(
      int givennumber /**< Number whose physical index shall be determined. */
      ) const;
   ...
   *  \return true iff number is contain in the index set */
  inline bool isMember(int _number /**< Number to be tested for membership. */
                       ) const;

 private:
  /* Updates an array of all numbers within the index set in correct order.*/
  returnValue updateNumberArray();

  std::vector<int>
      number; /**< Array to store numbers of constraints or bounds. */
  std::vector<int> next; /**< Array to store physical index of successor. */
  std::vector<int>
      previous; /**< Array to store physical index of predecossor. */
